...
class CameraRenderer(private val context: Context) : GLSurfaceView.Renderer {
    ...
    private var tintUniform = -1
    private var tintValue = 0f
    private var eglContext: EGLContext? = null
    private var vbo = -1

    fun setSurfaceTexture(st: SurfaceTexture) { surfaceTexture = st }
    fun setTint(value: Int) { tintValue = value.toFloat() / 100f }
    fun getEglContext(): EGLContext = eglContext ?: error("EGL context not ready")
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        tintUniform = GLES30.glGetUniformLocation(program, "uTint")
    ...
    fun drawToTarget(width: Int, height: Int) {
        ...
        surfaceTexture?.let { st ->
            ...
            GLES30.glUniform1f(tintUniform, tintValue)
            drawFullScreenQuad()
        }
    }
    ...
    private fun createProgram(): Int {
        val vs = loadShader(GLES30.GL_VERTEX_SHADER, VERTEX_SHADER)
        val fs = loadShader(GLES30.GL_FRAGMENT_SHADER, FRAGMENT_SHADER)
    ...
    companion object {
        private const val VERTEX_SHADER = "#version 300 es\nlayout(location=0) in vec4 aPos; layout(location=1) in vec2 aTexCoord; out vec2 vTexCoord; void main(){gl_Position=aPos;vTexCoord=aTexCoord;}"
